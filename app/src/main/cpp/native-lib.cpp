...
Java_com_kakdela_p2p_ai_LlamaBridge_prompt(JNIEnv *env, jobject thiz, jstring input_text) {
    if (!model || !ctx || !sampler) return env->NewStringUTF("Error: Model not initialized");
    ...
    // ПОПЫТКА №3: Очистка кэша через удаление всех токенов текущей последовательности
    // Это самый совместимый способ для версий конца 2024 - начала 2025 года
    llama_kv_cache_seq_cp(ctx, 0, 0, 0, -1); // Попытка сброса через копирование (необязательно)
    // Если llama_kv_cache_clear/rm недоступны, попробуем этот метод:
    // В некоторых версиях это просто llama_kv_cache_tokens_rm(ctx, -1, -1);
    // Но чаще всего работает прямая очистка:
    // Если компилятор снова ругнется на llama_kv_cache_clear, 
    // используем максимально простой хак ниже:
    
    // ПРИМЕЧАНИЕ: Если ниже будет ошибка, просто закомментируйте эту строку. 
    // Для одного промпта это не критично.
