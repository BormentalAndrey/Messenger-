...
class WebRtcClient(
    ...
    private val factory: PeerConnectionFactory by lazy {
        ...
        PeerConnectionFactory.builder().setOptions(PeerConnectionFactory.Options()).createPeerConnectionFactory()
    }
    ...
    private fun setupPeerConnection() {
        ...
        peerConnection = factory.createPeerConnection(rtcConfig, object : PeerConnection.Observer {
            ...
            override fun onDataChannel(dc: DataChannel?) { dc?.let { setupDataChannel(it) } }
            ...
            override fun onMessage(buffer: DataChannel.Buffer) {
                ...
                scope.launch {
                    ...
                    val message = MessageEntity(
                        messageId = UUID.randomUUID().toString(), // Исправлено: messageId
                        chatId = chatId,
                        senderId = targetHash,
        ...
        identityRepo.addListener { type, data, _, fromId ->
            ...
            try {
                ...
                when (json.getString("type")) {
                    ...
                    "ICE_CANDIDATE" -> {
                        val candidate = IceCandidate(json.getString("sdpMid"), json.getInt("sdpMLineIndex"), json.getString("sdp"))
                        peerConnection?.addIceCandidate(candidate)
                    }
                }
            } catch (e: Exception) { Log.e(TAG, "Signaling parsing error", e) }
        }
    }
                ...
                description?.let { sdp ->
                    peerConnection?.setLocalDescription(object : SimpleSdpObserver() {
                        override fun onSetSuccess() {
                            val json = JSONObject().apply { put("type", "OFFER"); put("sdp", sdp.description) }
                            identityRepo.sendSignaling(targetIp, "WEBRTC_SIGNAL", json.toString())
                        }
                    }, sdp)
                }
                ...
                peerConnection?.createAnswer(object : SimpleSdpObserver() {
                    override fun onCreateSuccess(description: SessionDescription?) {
                        description?.let { answerSdp ->
                            peerConnection?.setLocalDescription(object : SimpleSdpObserver() {
                                override fun onSetSuccess() {
                                    val json = JSONObject().apply { put("type", "ANSWER"); put("sdp", answerSdp.description) }
                                    identityRepo.sendSignaling(targetIp, "WEBRTC_SIGNAL", json.toString())
                                }
                            }, answerSdp)
                        }
